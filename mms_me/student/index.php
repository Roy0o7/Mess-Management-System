...
<?php session_start();
	...
	{
		$rollno = $_SESSION['userid'];						//the roll number of the student logging in
		if(isset($_REQUEST['cmid']))						//to check the current mess
			$cmid = $_REQUEST['cmid'];
		$connection = mysql_connect("localhost","root","");	//connection to database
		mysql_select_db("mms",$connection) or die("could not connect to database");
		$mess_num = 'AK0002';		//default mess id
		//checking notifications
			$result2 = mysql_query("select * from `notification` where `receiver`='$rollno' and `seenbyreceiver`=0");
			$countnot=$result2?mysql_num_rows($result2):0;
	}
	else
		{header('Location: ../index.php');}					//incorrect person accessing
         ...
         <div id="side-wrap">
    	  ...
    	  <div id="mid">
            ...
            <div id="header">
              ...
              <div id="tabs">
                <ul>
					<?php if(isset($_REQUEST['mess_num']))  // to check which mess is being displayed
							$mess_num = $_REQUEST['mess_num'];
					?>
					<?php 
					$result = mysql_query("select * from mess");
					while($mess_names = mysql_fetch_array($result))
					{
							$s1 = "<li><a href=\"index.php?rollno={$rollno}&mess_num={$mess_names['messid']}\"";
							$s2=($mess_names['messid']==$mess_num)?"class=\"active\">":"";
                   ...
                   <div class="post">
                    ...
                    <div class="postbody">
					  ...
					  <?php $query = "select * from student where rollno = '{$rollno}'";
						...
						<p><b>MessId</b> : <?php echo $mess_names['messid']; ?></p>
						<p><b>Owner</b> : <?php echo $mess_names['owner']; ?></p>
						<p><b>Phone No.</b> : <?php echo $mess_names['phoneno']; ?></p>
						<p><b>Email Id</b> : <?php echo $mess_names['emailid']; ?></p>
