<?php session_start();
		...
		if(isset($_REQUEST['editmess_submit']))
		{
			$mi=$_REQUEST['messid'];
			$newname=$_POST['name'];
			$newphno=$_POST['phno'];
	...
	// add a toupper function here
			...
			$add_mess_query="UPDATE `mms`.`mess` SET `name` = '$newname', `owner` = '$newowner', `phoneno` = '$newphno', `emailid` = '$newemail' WHERE `mess`.`messid` ='$mi'";
			$add_mess=mysql_query($add_mess_query);
			header('Location: showmesslist.php?success=1');
    	  ...
    	  <div id="mid">
            ...
            <div id="mid-content">
                   ...
                   <div class="post">
                    ...
                    <div class="postbody">
						<form id="editmess_form" action="editmess.php?messid=<?php echo $_REQUEST['messid'];?>" method="post">
							<table>
							<?php
							include("../connect.php");
							$sql_connect=mysql_connect($host,$user,$pass) or die("cannot connect to database.please try after sometime");
					...
					$q="select * from `mess` where `messid`='{$_REQUEST['messid']}'";
							...
							?>
								...
								<tr>
									<td>OWNER NAME:</td>
									<td><input type="text" maxlength="10" name ="owner" value="<?php echo $q_a['owner']?>"></td>
								</tr>
								<tr>
									<td>EMAIL ID:</td>
								...
								<tr>
									<td>PHONE NUMBER:</td>
									<td><input type="text" maxlength="100" name ="phno" value="<?php echo $q_a['phoneno']?>"></td>
								</tr>
							</table>
							<center><input type="submit" value="Update Mess Profile" name="editmess_submit">
						</form>
                    </div>
